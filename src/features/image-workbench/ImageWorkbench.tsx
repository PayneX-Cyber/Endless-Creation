...
function renderPromptText(value: string): ReactNode[] { return value.split(/(@\u56fe\d+)/g).map((part, index) => /^@\u56fe\d+$/.test(part) ? <mark className="image-studio__prompt-mention" key={index}>{part}</mark> : <span key={index}>{part}</span>); }
function getPromptMentionRange(value: string, position: number) { for (const match of value.matchAll(/@\u56fe\d+/g)) { const start = match.index ?? 0; const end = start + match[0].length; if (start < position && position < end) return { start, end }; } return null; }
...
export function ImageWorkbench() {
  ...
  const mentionedReferences = useMemo(() => references.filter((ref, index) => promptText.includes(getReferenceMentionLabel(index)) || promptText.includes(`@${ref.previewLabel}`) || promptText.includes(`@${ref.name}`)), [promptText, references]);
  const activeMentionIndex = Math.min(referenceMentionIndex, Math.max(references.length - 1, 0));
  ...
  function keepPromptCaretOutsideMention(value = promptText) { const textarea = promptTextareaRef.current; if (!textarea || textarea.selectionStart !== textarea.selectionEnd) return; const position = textarea.selectionStart; const range = getPromptMentionRange(value, position); if (!range) return; const next = position - range.start < range.end - position ? range.start : range.end; textarea.setSelectionRange(next, next); }
  function updatePrompt(value: string) { setPromptText(value); window.requestAnimationFrame(() => keepPromptCaretOutsideMention(value)); const shouldOpenMention = value.includes('@') && references.length > 0; setReferenceMentionOpen(shouldOpenMention); if (shouldOpenMention) setReferenceMentionIndex(0); setErrorMessage(''); if (status === 'generating') return; setStatus(value.trim() ? 'ready' : 'idle'); }
  function insertReferenceMention(reference: ReferenceImage) { const textarea = promptTextareaRef.current; const label = getReferenceMentionLabel(references.findIndex((item) => item.id === reference.id)); const start = textarea?.selectionStart ?? promptText.length; const end = textarea?.selectionEnd ?? start; const before = promptText.slice(0, start); const atIndex = before.lastIndexOf('@'); const replaceStart = atIndex >= 0 ? atIndex : start; const nextPrompt = `${promptText.slice(0, replaceStart)}${label} ${promptText.slice(end)}`; setPromptText(nextPrompt); setReferenceMentionOpen(false); setStatus(nextPrompt.trim() ? 'ready' : 'idle'); window.requestAnimationFrame(() => { promptTextareaRef.current?.focus(); const cursor = replaceStart + label.length + 1; promptTextareaRef.current?.setSelectionRange(cursor, cursor); }); }
...
<div className="image-studio__params" aria-label="生图参数"><div className="image-studio__model-menu" ref={modelMenuRef}><button className="image-studio__field image-studio__field--select image-studio__model-trigger" type="button" disabled={imageModelOptions.length === 0} aria-haspopup="listbox" aria-expanded={isModelMenuOpen} onClick={() => { if (imageModelOptions.length) setModelMenuOpen((open) => !open); }}><span>图片模型</span><strong>{imageModelLabel}</strong><ChevronDownIcon /></button>{isModelMenuOpen && imageModelOptions.length ? <div className="image-studio__model-dropdown" role="listbox" aria-label="图片模型">{imageModelOptions.map((option) => { const selected = option.value === config.modelId; return <button className={selected ? 'image-studio__model-option image-studio__model-option--active' : 'image-studio__model-option'} type="button" role="option" aria-selected={selected} key={option.value} onClick={() => { updateConfig({ modelId: option.value }); setModelMenuOpen(false); }}><span aria-hidden="true" /><strong>{option.label}</strong></button>; })}</div> : null}</div><SizeField value={config.size} onChange={(size) => updateConfig({ size })} /><div className="image-studio__field image-studio__field--quality"><span>质量</span><div className="image-studio__quality-group" role="group" aria-label="质量">{qualityOptions.map((option) => <button aria-pressed={config.quality === option} className={config.quality === option ? 'image-studio__quality image-studio__quality--active' : 'image-studio__quality'} key={option} onClick={() => updateConfig({ quality: option })} type="button">{option}</button>)}</div></div><CompactNumber label="数量" value={config.count} min={1} max={4} onChange={(count) => updateConfig({ count })} /></div>
            ...
            <label className="image-studio__prompt-area"><span>提示词</span><div className="image-studio__prompt-input-wrap"><div className="image-studio__prompt-render" aria-hidden="true">{renderPromptText(promptText)}</div><textarea className={promptText ? 'image-studio__prompt-input image-studio__prompt-input--masked' : 'image-studio__prompt-input'} ref={promptTextareaRef} aria-invalid={Boolean(promptError)} aria-label="提示词" maxLength={4000} onChange={(event) => updatePrompt(event.target.value)} onClick={() => window.requestAnimationFrame(() => keepPromptCaretOutsideMention())} onFocus={() => { const shouldOpenMention = promptText.includes('@') && references.length > 0; setReferenceMentionOpen(shouldOpenMention); if (shouldOpenMention) setReferenceMentionIndex(0); }} onKeyDown={(event) => { if (event.key === '@' && references.length) { setReferenceMentionOpen(true); setReferenceMentionIndex(0); } if (isReferenceMentionOpen && references.length && event.key === 'ArrowDown') { event.preventDefault(); setReferenceMentionIndex((index) => (index + 1) % references.length); } if (isReferenceMentionOpen && references.length && event.key === 'ArrowUp') { event.preventDefault(); setReferenceMentionIndex((index) => (index - 1 + references.length) % references.length); } if (isReferenceMentionOpen && references.length && event.key === 'Enter') { event.preventDefault(); insertReferenceMention(references[activeMentionIndex]); } if (event.key === 'Escape') setReferenceMentionOpen(false); }} onKeyUp={() => window.requestAnimationFrame(() => keepPromptCaretOutsideMention())} onSelect={() => window.requestAnimationFrame(() => keepPromptCaretOutsideMention())} placeholder="描述你想生成的图片，可输入 @ 来指定参考图..." value={promptText} /></div>{isReferenceMentionOpen && references.length > 0 ? <div className="image-studio__mention-menu" role="listbox" aria-label="选择参考图">{references.map((ref, index) => { const selected = mentionedReferences.some((item) => item.id === ref.id); const active = index === activeMentionIndex; return <button className={selected || active ? 'image-studio__mention-option image-studio__mention-option--active' : 'image-studio__mention-option'} key={ref.id} onMouseEnter={() => setReferenceMentionIndex(index)} onMouseDown={(event) => { event.preventDefault(); insertReferenceMention(ref); }} type="button" role="option" aria-selected={selected || active}>{ref.previewUrl ? <img src={ref.previewUrl} alt="" /> : <span aria-hidden="true" />}<strong>{getReferenceMentionLabel(index)}</strong><small>{ref.previewLabel}</small></button>; })}</div> : null}<div className={`image-studio__prompt-footer ${isReferenceDragging ? 'image-studio__prompt-footer--dragging' : ''}`} onDragEnter={handleReferenceDrag} onDragOver={handleReferenceDrag} onDragLeave={handleReferenceDragLeave} onDrop={handleReferenceDrop}><div className="image-studio__reference-list" aria-label="参考图列表">{references.length === 0 ? <span className="image-studio__empty-inline">暂无参考图。</span> : references.map((ref) => <div className={`image-studio__reference image-studio__reference--${ref.palette}`} key={ref.id} onClick={() => { if (ref.previewUrl) setPreviewReference(ref); }} onKeyDown={(event) => { if (ref.previewUrl && event.key === 'Enter') setPreviewReference(ref); }} role="button" tabIndex={ref.previewUrl ? 0 : -1}>{ref.previewUrl ? <img src={ref.previewUrl} alt="" /> : null}<span title={ref.name}>{getReferenceMentionLabel(references.findIndex((item) => item.id === ref.id))}</span><button aria-label={`移除${ref.name}`} onClick={(event) => { event.stopPropagation(); removeReference(ref.id); }} type="button">移除</button></div>)}</div><input ref={referenceInputRef} className="image-studio__file-input" accept="image/*" multiple onChange={handleReferenceUpload} type="file" /><button className="image-studio__upload-button" disabled={references.length >= MAX_REFERENCE_IMAGES} onClick={openReferencePicker} type="button"><UploadIcon /><span>上传参考图</span></button></div></label><div className="image-studio__count" aria-live="polite">{promptText.length}/4000</div>{promptError && <div className="image-studio__hint image-studio__hint--error" role="alert">{promptError}</div>}
