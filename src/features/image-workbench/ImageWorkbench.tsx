...
export function ImageWorkbench() {
  ...
  function updatePrompt(value: string) { setPromptText(value); const shouldOpenMention = value.includes('@') && references.length > 0; setReferenceMentionOpen(shouldOpenMention); if (shouldOpenMention) setReferenceMentionIndex(0); setErrorMessage(''); if (status === 'generating') return; setStatus(value.trim() ? 'ready' : 'idle'); }
