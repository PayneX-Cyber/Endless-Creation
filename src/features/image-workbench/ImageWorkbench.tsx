...
export function ImageWorkbench() {
  ...
  useEffect(() => {
    if (!isHistoryOpen) return;
    const targets = history.map((item) => item.results[0]).filter((result): result is GenerationResult => Boolean(result?.localPath && !result.imageUrl && !historyPreviewMap[result.id]));
