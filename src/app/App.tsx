...
const sidebarNavItems = [
  { id: 'projects', icon: '?', label: '????', active: true },
  { id: 'prompts', icon: '?', label: '????', active: false },
];
...
export function App() {
  ...
  return (
    <div className="app-shell" data-theme={theme}>
      <aside className="canvasflow-sidebar" aria-label="CanvasFlow ???">
        <header className="canvasflow-brand">
          ...
          <button className="canvasflow-collapse" aria-label="?????" type="button">?</button>
        </header>

        <nav className="canvasflow-nav" aria-label="CanvasFlow ??">
          ...
          <section className="canvasflow-nav__group" aria-label="????">
            ...
            <div className="canvasflow-subnav">
              {assetItems.map((item) => (
                <button className="canvasflow-subnav__item" key={item.label} type="button">
                  <span className="canvasflow-nav__icon" aria-hidden="true">{item.icon}</span>
                  <span>{item.label}</span>
                  <span className="canvasflow-badge" aria-label={`${item.label} ${item.count} ?`}>{item.count}</span>
                </button>
              ))}
            </div>
          </section>
