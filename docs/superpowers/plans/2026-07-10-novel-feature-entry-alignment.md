...
**状态**：2026-07-11 已实施并合入 `main`，提交 `11c79f4`。
...
- [x] **Step 1: 创建 novelExport.ts，迁入四 handler + 五纯函数**
...
- [x] **Step 2: 从 ChapterWorkbench 删除已迁出的函数与 handler**
...
- [x] **Step 4: 清理 ChapterWorkbench 不再使用的 storeZip import**
...
- [x] **Step 3: embedded 头部渲染「AI 分析章节」按钮**
...
- [x] **Step 1: NovelCreation 加意图与最近有效章 state**
...
- [x] **Step 2: openProjectWorkbench 支持携带意图 + 失败清意图**
...
- [x] **Step 4: 伏笔 tab 传 analyze props + 打开选择器**
...
- [x] **Step 5: ChapterWorkbench 消费 initialPanel + 返回 tab**
...
- [x] **Step 6: ChapterWorkbench 上报最近有效章（选中有正文章 + 空章生成出正文）**
...
- [x] **Step 7: 工作台「项目详情」返回按 returnTab**
...
- [x] **Step 1: ProjectViewTab 类型去 emotion**
...
- [x] **Step 2: PROJECT_VIEW_TABS 删情感曲线项**
...
- [x] **Step 5: 删 icons.tsx 里 ChartIcon 定义**
...
- [x] **Step 6: 删 NovelCreation.css 情感曲线样式**
...
- [x] **Step 1: 双端 build 终检**

Run: `cd "F:/AIProject/Endless Creation" && npm run build`
Expected: exit 0（renderer tsc+vite + electron tsc 全绿）。

- [x] **Step 2: git diff --check + 无 schema/IPC 改动审计**
